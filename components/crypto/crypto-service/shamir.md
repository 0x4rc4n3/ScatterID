# Component Technical Specification: shamir.py (KMS Crypto-Service Copy)

## 1. Purpose & Core Responsibility
- Serves as the core Shamir Secret Sharing sharding mathematical wrapper.
- Splits binary raw payloads (secrets) into multiple hexadecimal-encoded fragments (shards) and appends a secure SHA-256 checksum to each share to enable storage node integrity auditing.
- Reconstructs secrets from threshold subsets of shares.
...
- **Allowed Outbound (Who this file can talk TO):** 
  - `sslib.shamir` third-party library API methods.
- **Explicitly Denied Outbound:** 
  - All other external connections.

## 3. Function & Method Manifest
- **`split_secret(secret, n, k)`**
  - **Purpose:** Splits secret bytes into $n$ parts, requiring a threshold of $k$ to recover. Appends a SHA-256 checksum to each output share (format `share_value:checksum`).
  - **Inputs & Sanitization:** 
    - `secret`: Must be non-empty `bytes`.
    - `n`, `k`: Must be positive integers, validating that threshold $k \le n$.
  - **Outputs:** Dict containing the SSS configurations and hex-encoded, checksummed shares.
