...
if [ ! -f "product/crypto-service/certs/crypto-service.crt" ] || [ ! -f "product/crypto-service/certs/ca.crt" ]; then
    echo "[+] Generating internal TLS certificates..."
    bash product/crypto-service/certs/generate_certs.sh
else
...
if ! docker ps --format "{{.Names}}" | grep -q "peer0.issuer.scatterid.com"; then
    ...
    (cd product/blockchain/fabric-network && ./start.sh)
else
...
docker exec -i -e VAULT_ADDR=http://127.0.0.1:8200 -e VAULT_TOKEN=scatterid-vault-root-token vault.scatterid.com vault policy write issuer-policy - < product/crypto-service/crypto-service/vault/policies/issuer-policy.hcl >/dev/null
...
docker compose restart verification-api operator-console >/dev/null 2>&1 || true
