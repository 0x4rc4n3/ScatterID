...
- Manages dynamic, tenant-isolated local SQLite databases per shard node for absolute separation of zero-trust states.

## 2. Inbound & Outbound Communication Whitelist
- **Allowed Inbound (Who can talk TO this file):** 
    - Verification API Gateway (`verification-api`) via HTTP on Port 3000, authorized using Bearer tokens (`SHARD_NODE_API_KEY`) and tenant routing headers (`X-Tenant-ID`).
- **Explicitly Denied Inbound:** 
    - All other callers and external connection routes.
- **Allowed Outbound (Who this file can talk TO):** 
    - Dynamic tenant SQLite database files (`<tenant_id>_node_[NODE_INDEX].db`).
    - Fallback default system SQLite database file (`node_[NODE_INDEX].db`).
...
- **`requireTenant(req, res, next)`**
    - **Purpose:** Extracts, sanitizes, and enforces the dynamic tenant ID routing header.
    - **Inputs & Sanitization:** `X-Tenant-ID` header. Validated against regular expression `/^[a-zA-Z0-9_\-]+$/`.
    - **Outputs:** Stores tenantId in `req.tenantId` and calls next middleware or returns HTTP 400.
    - **Error States & Handling:** Rejects missing or malformed header values with HTTP 400.

- **`getTenantDb(tenantId)`**
    - **Purpose:** Dynamic connection manager that lazily resolves, opens, initializes, and caches tenant-specific SQLite connections and pre-compiled statements.
    - **Inputs & Sanitization:** `tenantId` (string).
    - **Outputs:** Connection entry containing `{ db, stmts }`.

- **`POST /shard`**
    - **Purpose:** Accepts and stores a single Shamir share fragment and the associated credential metadata record under the tenant's database context.
...
- **`GET /shard/:credentialId`**
    - **Purpose:** Retrieves a stored shard value and metadata for a specific credential ID from the tenant's database.
    - **Headers Required:** `X-Tenant-ID`
...
- **`POST /update-status`**
    - **Purpose:** Updates anchor status and transaction details for a stored credential in the tenant's database.
    - **Headers Required:** `X-Tenant-ID`
    - **Inputs & Sanitization:** 
...
- **`GET /integrity`**
    - **Purpose:** Evaluates database health by comparing calculated SHA3-256 hashes of stored tenant shares against their recorded hashes.
    - **Headers Required:** `X-Tenant-ID`
...
- Implements strict tenant header validation (`X-Tenant-ID`) to block partition-bypass attempts.
