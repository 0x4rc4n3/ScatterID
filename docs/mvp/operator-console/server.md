# Component Technical Specification: server.js (Operator Console Backend)
...
- **Allowed Outbound (Who this file can talk TO):** 
    - Verification API Gateway (`http://verification-api:3000`) via HTTP requests, authorized with Bearer key (`GATEWAY_API_KEY`).
...
- **`getGatewayHeaders()`**
    - **Purpose:** Injects Content-Type and `Authorization: Bearer <GATEWAY_API_KEY>` headers for secure outgoing API queries.
    - **Outputs:** Headers configuration object.
...
- Enforces Bearer Authorization when calling the Verification API.
